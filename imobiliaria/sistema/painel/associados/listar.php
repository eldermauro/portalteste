...
if($nivel_usuario == 'Corretor'){
	$query = $pdo->query("SELECT * FROM associados where corretor = '$id_usuario' ORDER BY id desc");
}else{
	$query = $pdo->query("SELECT * FROM associados ORDER BY id desc");
}
...
//retirar quebra de texto do obs
		...
		<big><a href="#" onclick="editar('{$id}', '{$nome}', '{$apelido}', '{$telefone}', '{$doc}',  '{$valor}', '{$pg}', '{$status}',  '{$corretor}',  '{$pessoa}')" title="Editar Dados"><i class="fa fa-edit text-primary"></i></a></big>

		<big><a href="#" onclick="mostrar('{$nome}', '{$apelido}', '{$telefone}', '{$doc}',  '{$valor}', '{$pg}', '{$status}',  '{$nome_corretor}',  '{$data_cadF}',  '{$pessoa}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function editar(id, nome, apelido, telefone, doc,  valor, pg, status,  corretor,  pessoa){
	...
	function mostrar(nome, apelido, telefone, doc, valor, pg, status, corretor, data_cad, pessoa){
