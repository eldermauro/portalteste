...
if($alterou_data == 'Sim'){
	...
	$query = $pdo->query("SELECT * from $pagina where usuario_lanc LIKE '$id_usuario' order by id desc ");
}
...
$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
if(@count($res2) > 0){
	$nome_locatario = $res2[0]['nome'];
}else{
	$nome_locatario = 'Sem Registro';
}
...
echo <<<HTML
			<tr> 
				...
				<td>
					<big><a class="{$ocultar}" href="#" onclick="editar('{$id}', '{$descricao}', '{$comprador}', '{$locatario}','{$valor}','{$data_venc}','{$frequencia}','{$saida}','{$arquivo}')" title="Editar Dados"><i class="fa fa-edit text-primary "></i></a></big>

					<big><a href="#" onclick="mostrar('{$id}', '{$descricao}', '{$nome_comprador}', '{$nome_locatario}','{$valor}','{$data_lancF}','{$data_vencF}','{$data_pgtoF}','{$nome_usu_lanc}','{$nome_usu_pgto}','{$nome_frequencia}','{$saida}','{$arquivo}','{$pago}')" title="Ver Dados"><i class="fa fa-info-circle text-secondary"></i></a></big>
...
<script type="text/javascript">
	...
	function editar(id, descricao, comprador, locatario, valor, data_venc, frequencia, saida, arquivo){
	...
	function mostrar(id, descricao, comprador, locatario, valor, data_lanc, data_venc, data_pgto, usuario_lanc, usuario_pgto, frequencia, saida, arquivo, pago){
		...
		if(comprador != "Sem Registro" && locatario == "Sem Registro"){
			$('#pessoa_mostrar').text(comprador);
			$('#tipo_pessoa_mostrar').text('Comprador');
		}else if(locatario != "Sem Registro" && comprador == "Sem Registro"){
